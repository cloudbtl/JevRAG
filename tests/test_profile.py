...
def test_choose_profile_lays_fields_out_as_option_cards_with_coverage_and_keeps_the_helpful_ones():
    ...
    p = choose_profile("2019년 Alpha타워 임차인별 임대료", jev, cov)
...
def test_hop_cards_show_only_the_profiled_fields():
    opt = {"kind": "document", "id": "doc_1", "label": "Rent Roll", "weight": 1,
           "document": {"documentType": "xlsx", "fileSize": 1, "createdAt": "2019-11-01T00:00:00Z", "sourceRef": "RealEstate/Alpha/rr.xlsx"},
           "cards": [{"producer": "cloudbtl-baseline", "payload": {"pageCount": 2, "pageLabels": ["RentRoll_Alpha", "층별"], "headline": "ALPHA 렌트롤", "metadata": {"division": "real-estate"}}},
                     {"producer": "llm-cards", "payload": {"summary": "임차인별 보증금·임대료", "docType": "렌트롤", "period": "2019-11", "entities": ["Alpha타워", "Tenant A"], "topics": ["임대"]}}]}
    ...
    assert narrow.facts["entities"] == "Alpha타워, Tenant A" and narrow.facts["path"] == "RealEstate/Alpha"
